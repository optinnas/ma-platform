...
export function MultiSelect<T, U = T>({
    ...
    const { valid, invalid } = (value ?? []).reduce(
        (a, v) => {
            ...
            if (option) {
                ...
                a.valid.push(<Fragment key={valueKey}>{getSelectedOptionDisplay(option)}</Fragment>)
            } else {
                a.invalid.push(v)
            }
            return a
    ...
    return (
        ...
        >
            ...
            <Listbox.Button
                className={clsx("select-button", size, buttonClassName)}
            ...
            >
                <span className="select-button-label">
                    {value?.length ? (
                        <>
                            {valid}
                            {!!invalid.length && <span>{`+${invalid.length} Invalid Values`}</span>}
                        </>
                    ) : (
                    ...
                    return (
                        <Listbox.Option
                            ...
                            className={({ active, selected }) =>
                                clsx("select-option", active && "active", selected && "selected")
                            }
                        >
...
MultiSelect.Field = function MultiSelectField<X extends FieldValues, P extends FieldPath<X>>({
    ...
    return (
        <MultiSelect {...rest} {...field} required={required} error={fieldState.error?.message} />
    )
}
